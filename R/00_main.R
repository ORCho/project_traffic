...
weather<-read.table("./data/wheather\ data\ de\ Bilt.txt", header=F, sep=",", stringsAsFactors=F)


weather <- read.table(paste(datadir,"wheather data de Bilt.txt",sep=""), header=F, sep=",", stringsAsFactors=F)
...
sunrise<-read.csv2("./data/sunrise\ times2013.csv", stringsAsFactors = F)
...
################### ACCIDENT DATA #####################
#######################################################
accidents <- read.table("./data/ongevallen\ 2013.txt", header=T, sep=",", stringsAsFactors = F)
